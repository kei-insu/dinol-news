...
- **main**: 8/11 브리핑을 발행했다(`18ff8f1`, 롤백 지점 `797c70f`). `validate.py` ERROR 0·WARN 0.
  구성 균형은 지역 2:2·매체 8종 각 1건·그라디언트 8종 고유·EN 4장 필드 완비로 통과했으나
  **em대시 5건이 검출돼 이 세션에서 치환했다** — 전부 EN 카드(1·2·5·6)의 영문 `designer`·`comment` 필드였고,
  각각 유일 매치를 확인한 뒤 대시를 마침표 또는 콜론으로 바꿨다. 치환 후 카드 본문 0건·`validate.py` 재실행 ERROR 0.
  게재일 예외 1건 승인 — 카드8 벤처스퀘어 `2026-07-31`(11일 경과, 기준 1주). 나머지 7장은 08.07~08.10.

- **astro**: 병합이 **충돌 0건**으로 끝났다(`72e27d1`, 3파일·411 insertions).
  8/10 세션에서 문서 개편분을 전부 병합해 둔 효과다 — 이후 매일 병합은 브리핑 3파일만 들어온다.
  이어 카드 8장을 추출했고(`ad98126`, 게이트 `[0]`~`[10]` 전수 통과 46.9s),
  게이트 `[1]` UNKNOWN 1건(`PetaPixel`)은 `source_rules.json` 71→72종으로 해소했다(추가만·삭제 0·변경 0 대조 검증).
  `PetaPixel` 은 4항목 **전부 통과**라 제약 없이 3사본에 등록했다(`25310c3`).

- **⚠️ 배포 후 발견된 규칙 미충족 1건** — `daily_import.py [5]` 가 **전체 ★5 0장**을 잡았다
  (`policy.md §7-1` 「방법론·툴 사용법 0순위」 미충족, `§9-1` 기록 대상). AI [4,4,3,1] high45 2 / Design [3,3,2,2] high45 0.
  **배포 전 수동 검증에서는 놓쳤다** — 세션 체크리스트에 「AI 섹션 ★4~5 최소 1장」만 있고 전체 ★5 조건이 없다(§4).
  배포는 되돌리지 않고 노션 배포 행에 기록했다.

- **오늘 커밋 4건** — `main` 1건(`18ff8f1`) / `astro` 3건(`72e27d1`·`ad98126`·`25310c3`).
  노션 작업 일지 3행 기입 완료(커밋 타임스탬프 `%cI` 실측값 사용, 조회 확인).
...
- [x] ~~8/10·8/11 브리핑 발행~~ — **완료**(`797c70f`·`18ff8f1`). 롤백 지점 기록·commit-pinned 반영 확인 포함

**astro**

- [ ] **(즉시)** 배포 세션 체크리스트에 **「전체 ★5 최소 1장」** 조건을 추가한다 — `policy.md §7-1` 이 근거인데 세션 지침의 ③ 구성 균형 표에는 「AI 섹션 ★4~5 최소 1장」만 있다. 8/11 배포에서 이 조건을 놓쳤고 `daily_import.py [5]` 가 배포 **후에** 잡았다. 검증이 뚫린 지점이므로 체크리스트를 고쳐야 재발하지 않는다
- [ ] `routine_instruction.md` §3 헤딩 소스 카운트 정정 (노션 백로그 등록됨) — AI 국내 16/실제16 · **AI 해외 24/실제26** · Design 국내 22/실제24 · Design 해외 35/실제43. 세션 6·7에서 쓴 22·24 는 기존 표기 +1 이지 정확한 값이 아니다. 정정 시 4개 블록을 함께 맞추고 루틴 UI(사본 ③)도 마커 아래 전체 교체
...
- [ ] Design 섹션 ★4~5 0장이 **이틀 연속**이다(8/10 ★3·3·3·3 → 8/11 ★3·3·2·2). 방법론·툴 콘텐츠가 Design 축에서 안 잡히는 원인을 조사할지 결정한다. 루틴 §3 Design 축에 UX·툴 소스는 있으나(UX Collective·NN/g·Smashing) 실제로 카드가 되지 않는다
...
- [ ] 노션 작업 일지 `날짜` 순서 역전 정정 여부 결정 (§6). `0067a5e`·`b75f4c8` 2행이 커밋 시각과 어긋나고 `브랜치` 가 비어 있다. **세션 6에서 기입한 6행도 컨테이너 시계 기반 추정값이라 실제와 어긋났을 수 있다** — 세션 7부터는 `git log -1 --format=%cI` 실측값만 쓴다
- [ ] `459b216`(HANDOFF 세션 3 재갱신) 노션 작업 일지 미기입 — 세션 5·6·7 에서도 처리하지 않았다
...
- push 상태(2026-08-11 00:15 조회) — `astro` 미push 4건(`18ff8f1`·`72e27d1`·`ad98126`·`25310c3`) · `main` 0건(`18ff8f1` 배포 시 push 됨) · `5b-2-emulator` 브랜치 자체가 원격에 없음. **`18ff8f1` 은 `origin/main` 에는 있으나 `origin/astro` 에는 없어 ahead 카운트에 잡힌다** — 브랜치별로 세어야 한다
