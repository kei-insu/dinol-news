...
| 최종 갱신 | 최근 변경 |
|---|---|
| 2026-08-10 | ADR 0001 링크 상대경로 정정(adr/ → ../adr/) |
| 2026-08-09 | 2026-07-15 Auth 전환의 판단 근거를 ADR 0001 로 분리 |
...
|---|---|---|---|---|---|---|
| 2026-07-15 | 높음 | 마스터(운영자) 비밀번호 공개 노출 | `assets/dinol-firebase.js`에 `const ADMIN_PW = "<값 생략>"` 하드코딩. public repo + 브라우저에서 그대로 조회되어 **누구나 마스터 비번을 읽어 모든 글/댓글을 수정·삭제 가능**했음 | 클라이언트 JS에 인증 비밀을 둠. 마스터 권한 판별을 클라이언트에 위임(클라이언트 검증은 UX 편의일 뿐 보안 경계가 아님) | **코드 조치 완료**: `ADMIN_PW`·`ADMIN_HASH` 상수 제거(잔여 참조 0), 운영자 판별을 Firebase Auth 세션으로 이전. 판단 근거·검토했으나 버린 안은 [ADR 0001](../adr/0001-admin-auth-firebase-session.md). 잔여 작업(Firebase 콘솔 Auth 활성화·운영자 계정 생성·UID 규칙 반영)은 노션 백로그 소관 | 진행중 |
