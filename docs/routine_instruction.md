...
| 최종 갱신 | 상태 |
|---|---|
| 2026-07-10 | ⚠️ 일부 구버전(아래 참조) + 5번 2항에 발행 중복 검증(published_urls.json) 추가 |
...
2. **과거 발행 중복 검증 (필수)** — 기사 수집 후, 발행 URL 대장을 읽어 **이미 있는 URL은 카드에서 제외**한다(같은 기사 재발행 금지).
   - 대장 위치(둘 중 접근 가능한 것): `https://raw.githubusercontent.com/kei-insu/dinol-news/main/published_urls.json`
   - 수집한 각 기사 URL이 대장에 있으면 → 그 카드 **제외**하고 다른 기사로 대체.
   - 최종 카드 확정 후 → 새로 발행하는 URL들을 `{URL: 오늘날짜(YYYYMMDD)}` 형태로 대장에 **추가**한다.
   - 로컬에서 대장 재생성·점검: `python scripts/build_published_urls.py`
3. 게재일이 수집 기간 내인지 확인
4. 카드 타이틀·내용이 링크 페이지와 일치 (목록·홈·태그 페이지 URL 금지)
5. 빈 링크·깨진 링크 없음
