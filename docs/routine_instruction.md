...
### 3-1. 선별 정책 (후보 풀 → 디자인 관련도 가중 → 매체·날짜 분산) — 2026-07-20
...
- **매체 분산.** 한 매체에서 여러 건 몰아오지 않는다. 섹션당 동일 매체 1건 권장(불가피할 때만 최대 2). 계열 매체(예: 디자인플러스·헤이팝 = 디자인하우스 계열)는 같은 소스로 센다.
- **날짜 분산.** 수집 창 안에서 같은 날짜 기사만 몰지 않는다.
- **최신성 vs 관련도.** 신선한 KR 건이 얇으면 관련도·품질을 우선한다(며칠 지난 건 허용). 단 그 이유를 §9-1에 한 줄 남긴다.
