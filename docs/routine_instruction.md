...
2. **과거 발행 중복 검증** — `published_urls.json`(발행 URL 대장)을 읽어, 수집한 각 기사 URL이 **이미 있으면 제외**한다(같은 기사 재발행 금지). 최종 카드 확정 후, 새 URL들을 `{URL: 오늘날짜}`로 이 파일에 추가한다. (로컬 재생성: `python scripts/build_published_urls.py`)
3. 게재일이 수집 기간 내인지 확인
