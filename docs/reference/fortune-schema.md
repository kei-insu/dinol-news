...
## 6. 데이터 소스 API — freehoroscopeapi

> 출처: `https://freehoroscopeapi.com/` · 확인일 **2026-08-03**
> 아래는 공식 페이지 기재 내용이다. 실제 호출로 검증한 항목은 없다.

### 확인됨 — 공식 문서에 명시

| 항목 | 값 |
|---|---|
| 엔드포인트 | `GET /api/v1/get-horoscope/daily` · `/weekly` · `/monthly` |
| 파라미터 | `sign` (필수) |
| 예시 | `https://freehoroscopeapi.com/api/v1/get-horoscope/daily?sign=aquarius` |
| 응답 | `{"data":{"date","period","sign","horoscope"}}` — `period`는 `daily`\|`weekly`\|`monthly` |
| 지원 별자리 | 12종 (Aries ~ Pisces) |

**주는 것은 운세 텍스트(영어)뿐이다.** 응답 필드가 4개뿐이므로 별점·확률·궁합·색·폰트는 API가 주지 않는다 → 전부 루틴이 생성한다(§3~§4).

### 미확정 — 실제 호출로 확인 필요

| 항목 | 현재 근거 | 확인 방법 |
|---|---|---|
| 인증 필요 여부 | 공식 예시 요청에 인증 파라미터·헤더가 **표시되지 않음**. 항상 불필요하다는 보장은 아님 | 키 없이 호출해 200 응답 확인 |
| `sign` 대소문자 | 공식 페이지에 대소문자를 구분하지 않는다는 **문구는 있으나** 실동작 미검증 | `Aries`·`ARIES`·`aries` 3형태 호출 대조 |
| 에러 응답 형식 · HTTP 상태 코드 | 기재 없음 | 잘못된 `sign`으로 호출 |
| rate limit | 기재 없음 | 연속 호출 시 응답 헤더 확인 |

### 미확정 — 공식 이용조건 확인 필요

- 이용약관·콘텐츠 라이선스가 공식 페이지에 **없다.** 호출해서 알 수 있는 값이 아니므로 운영자 문의 또는 약관 페이지 확인이 필요하다.
- §5의 "원문 재게시 금지"는 법적 근거가 확인된 사항이 아니라 **프로젝트 내부의 보수적 운영 방침**이다.

### 운영 정책 (API 사양 아님)

- 개인(`sd416`) 운영 무료 서비스라 SLA·가용성 보장이 없다 → **실패 시 폴백 필수.** 직전 `fortune.json` 유지 또는 date-seeded 생성
- 응답이 영어이므로 번역·재해석 단계를 거친다(§5)
