...
| `routine_instruction.md` | 루틴 이해·수정 | 브리핑 자동생성 절차(WebSearch 전용). 발행 중복 대조 관문 · 카드 스키마 · 별점 루브릭 · §3 확장 소스 목록 · §10 발행 게이트 | 08-06 |
...
| 등록일 | 항목 | 비고 |
|---|---|---|
| 2026-08-06 | 루틴 UI 프롬프트 반영 확인 | `routine_instruction.md` 08-06판(확장 소스 목록)을 루틴 UI·노션 첨부에 반영했는지 미확인. 미반영 시 루틴은 여전히 구버전 5곳만 돈다 |
| 2026-08-06 | designboom 계열 확인 | designboom–ArchDaily(NZZ/Architonic) 계열 여부 1차 출처 미확인. 확인 전까지 계열 합산에서 제외. 확인되면 `news_sources.md`에 명시 |
| 2026-08-06 | 중국 소스 URL 확정 | `news_sources.md` §10 Design360°·TOPYS URL 미확정. 확인 전까지 자동 등록 금지, 검색어로만 사용 |
