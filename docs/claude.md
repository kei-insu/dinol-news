...
| 최종 갱신 | 최근 변경 |
|---|---|
| 2026-08-01 | 3자 검토 프로토콜 신설 + main 예외 3종 명시 + 커밋 전제조건 + 저장 경로 표기 형식 고정 |
...
AI가 함께 작성한 코드는 사람이 쓴 코드보다 약 1.7배 많은 문제를 포함하고
보안 취약점은 최대 2.7배 높다는 실측(CodeRabbit, PR 470건)이 있으며,
QA는 바이브코딩에서 가장 자주 간과되는 영역이다(ICSE 2026, 문헌 101건).

다만 **검토가 결함 찾기에만 쏠리면 명세가 계속 비대해진다.**
실측: `build_published_urls.py` 6회 · `daily_import --redo` 8회 · 5-A-2 명세 12회 이상.
매번 항목이 늘기만 하고 줄어든 적이 없었다. 이것이 **바이브코딩 둠 루프**다.
...
| 전환 방향 | 실제 사례 |
|---|---|
| 다른 설계로 갈아탄다 | `--redo`가 8회차에 "레포 직접 추출 후 복구" → **"임시 디렉터리 선추출"**로 바뀌자 복구 로직이 단순해지고 논의가 끝났다 |
| 범위를 쪼갠다 | 5-A를 구조(5-A-1)와 로직(5-A-2)으로 분리 |
| 직접 코드를 작성한다 | 명세로 설명하는 것보다 코드를 쓰는 게 빠른 경우 |
...
5-A-2가 `likes.js` + `likes-core.js` + 규칙 + CLI + 테스트 2종 + 검증 3종이었고,
그래서 빈틈이 계속 나왔다.
...
- **반드시 헤딩(`###`)으로 쓴다.** 굵은 글씨 한 줄은 앞 블록(표·문단)에 붙어 렌더링에서 삼켜진다. 2026-08-01에 실제로 사라져 사용자가 경로를 못 본 사고가 있었다.
- 헤딩 앞뒤로 **빈 줄을 둔다.**
