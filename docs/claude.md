...
| 최종 갱신 | 최근 변경 |
|---|---|
| 2026-08-02 | main 예외 3종 명시 + 커밋 전제조건 + 저장 경로 표기 형식 고정 |
...
**표기 형식 — 아래를 그대로 쓴다 (2026-08-02 고정)**

```
### 📁 저장 경로 — dinol-news > <폴더> 폴더
```

- **반드시 헤딩(`###`)으로 쓴다.** 굵은 글씨 한 줄은 앞 블록(표·문단)에 붙어 렌더링에서 삼켜진다. 2026-08-02에 실제로 사라져 사용자가 경로를 못 본 사고가 있었다.
- 헤딩 앞뒤로 **빈 줄을 둔다.**
