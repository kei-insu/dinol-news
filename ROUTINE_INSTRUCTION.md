...
영문 기사는 위 기본(영문) + 각 `-kr`(한국어) + `data-title-kr`(토글 유발)을 넣는다. `data-comment`도 EN 카드는 `data-comment-kr`를 함께 넣는다. **단 `data-impact-score`(별점)와 `data-category`는 언어 무관이라 `-kr` 불필요.**
