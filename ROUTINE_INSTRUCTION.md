...
<a class="card" href="[URL]" target="_blank"
   ...
   data-recommend="[추천]"
   data-comment="[💬 큐레이션 코멘트 · 오리지널 논평 2~3문장]">
...
<a class="card" href="[URL]" target="_blank"
   ...
   data-comment="[💬 큐레이션 코멘트 · 한국어 논평 2~3문장 · 언어 무관]"
...
카드 클릭 시 뜨는 팝업 필드 순서: **한 줄 요약 → 인사이트 포인트 → 디자이너 관점 → 실무 영향도(별점) → 추천 → 💬 큐레이션 코멘트.** 각 카드에 `data-*`로 담는다. **속성 이름은 `dinol.js`가 읽는 값과 정확히 일치해야 한다.**
- **`data-category`** — 카테고리 (언어 공유, 예: `AI · UX · Tool`, `디자인 · 브랜딩`)
- **`data-summary`** — 한 줄 요약 (1문장)
- **`data-points`** — 인사이트 포인트 2~3개, **`|`(파이프)로 구분**
- **`data-designer`** — 디자이너 관점 (1~2문장)
- **`data-impact-score`** — 실무 영향도 **별점**, **정수 1~5** (⚠️ 텍스트 아님. `data-impact` 텍스트는 팝업에 렌더되지 않는다. 값이 0/빈값이면 항목 자동 숨김)
- **`data-recommend`** — 추천 (한 줄)
- **`data-comment`** — 💬 **큐레이션 코멘트**, 오리지널 논평 2~3문장. **이 사이트의 핵심 부가가치(AdSense).** 언어 무관 단일 속성이며, 문단 구분은 `\n\n` 사용(`white-space: pre-line`).

영문 기사는 위 기본(영문) + 각 `-kr`(한국어) + `data-title-kr`(토글 유발)을 넣는다. **`data-impact-score`와 `data-comment`, `data-category`는 언어 무관이라 `-kr` 불필요.**
